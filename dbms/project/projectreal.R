...
tableCreate <- function() {
  ...
  sqlExecute(db,"create table secondcluster (clusterId int not null,inc_avgMax number(20,3), constraints pk_cluster primary key (clusterId) )")
  ...
  sqlExecute(db,"CREATE TABLE firstcluster_contents (clusterId int not null, dates int not null,diff number(20,3), sequence int,constraint pk_firstcluster_contents primary key (clusterId,dates), constraint fk_firstcluster_contents foreign key (clusterId) references firstcluster_map (clusterId) on delete cascade)")
  ...
  sqlExecute(db,"CREATE TABLE stock_contents (symbol varchar2(20) not null, dates int not null,diff number(20,3), sequence int,constraint pk_stock_contents primary key (symbol,dates), constraint fk_stock_contents foreign key (symbol) references stock (symbol) on delete cascade)")
...
secondcluster_insert <- function(){
  ...
  kcluster <- kmeans(clusterMatrix,20)
  kcluster$cluster[1]
  ...
  result = t(result)
  train = rbind(result)
  query <- paste0("select a.inc_avgMax from secondcluster a,(select secondfk from firstcluster_map where clusterid=",savedid[1],") b where b.secondfk=a.clusterid")
  ...
  for(i in 2:length(savedid))
  {
    ...
    query <- paste0("select a.inc_avgMax from secondcluster a,(select secondfk from firstcluster_map where clusterid=",savedid[i],") b where b.secondfk=a.clusterid")
    cl[index] = sqlQuery(db,query)
    index=index+1;
  }
  
  # make test set 
  symbols <- sqlQuery(db,"select distinct symbol from stock_contents order by symbol")
...
firstcluster_insert <- function() {
    ...
    kcluster <- kmeans(pointMatrix,35)
...
db <<- odbcConnect("jk","test","powerkk",believeNRows=FALSE)
tableCreate()
removeImperfect()
tableDrop()
odbcCloseAll()
