                                    ...
                                    const int ny)
{
    ...
    unsigned int iy = blockDim.y * blockIdx.y + threadIdx.y;
        ...
        int newX = tid%ny;
        int newY = tid/ny;
    unsigned int ti = newY * ny + newX; // access in rows
    unsigned int to = newX * nx + newY; // access in columns

    if (newX + 3 * blockDim.y  < ny && newY < nx)
    {
        out[ti]                = in[to];
        out[ti +   blockDim.y] = in[to +   blockDim.y * nx];
        out[ti + 2 * blockDim.y] = in[to + 2 * blockDim.y * nx];
        out[ti + 3 * blockDim.y] = in[to + 3 * blockDim.y * nx];
    }
}
