...
__global__ void matricMul (int *A, int *B, int *C, int nx)
{
...
int main (int argc, char **argv)
{
    ...
    // change the block configuration
    for (dim = 32; dim != 0 ; dim /=2)
    {
        ...
        matricMul<<<grid,block>>> (d_A,d_B,d_C,nx);
        ...
        {
            printf("not correct answer block : [%d,%d] grid : [%d,%d]\n",block.y,block.x,grid.y,grid.x);
            exit(1);
        }
        else 
        {
