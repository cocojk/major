...
int main(int argc, char **argv)
{
    ...
    CHECK(cudaMalloc((void **)&dANnzPerColumn, sizeof(int) * M));
    ...
    // Compute the number of non-zero elements in A and compute the number of non-zero elements in A per column
    // return : cusparseStatus_t 
    ...
    CHECK_CUSPARSE(cusparseDnnz(handle, CUSPARSE_DIRECTION_COLUMN, M, N, Adescr,
                                dA, M, dANnzPerColumn, &totalANnz));
    ...
    // parameter : void** devPtr, size_t size 
    // allocates (size) bytes of lineare memory on the device and returns in (*devPtr) a pointer to the allocated memory 
    CHECK(cudaMalloc((void **)&dCscValA, sizeof(double) * totalANnz));
    CHECK(cudaMalloc((void **)&dCscRowIndA, sizeof(int) * totalANnz));
    CHECK(cudaMalloc((void **)&dCscColPtrA, sizeof(int) * (N + 1)));

    // Convert A from a dense formatting to a CSC formatting, using the GPU
    // return : cusparseStatus_t 
    ...
    CHECK_CUSPARSE(cusparseDdense2csc(handle, M, N, Adescr, dA, M, dANnzPerColumn,
                                      dCscValA, dCscRowIndA, dCscColPtrA));

    // Perform matrix-matrix multiplication with the CSC-formatted matrix A
    // csc is csr transposed. so we can use csr matrix multiplication with CUSPARSE_OPERATION_TRANSPOSE
    ...
    CHECK_CUSPARSE(cusparseDcsrmm(handle, CUSPARSE_OPERATION_TRANSPOSE, N,
                                  M, M, totalANnz, &alpha, Adescr, dCscValA,
                                  dCscColPtrA,dCscRowIndA, dB, N, &beta, dC,
                                  M));
    ...
    CHECK(cudaFree(dB));
    CHECK(cudaFree(dC));
    CHECK(cudaFree(dANnzPerColumn));
    CHECK(cudaFree(dCscValA));
    CHECK(cudaFree(dCscRowIndA));
    CHECK(cudaFree(dCscColPtrA));
